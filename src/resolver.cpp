...
void Resolver::visit(const This *expr)
{
    ...
    {
        Pulse::error(expr->keyword, "Cannot use 'this' outside of a class.");
        return;
    }
...
void Resolver::declare(const Token name)
{
    ...
    {
        Pulse::error(name, "Variable with this name already declared in this scope.");
    }
...
void Resolver::visit(const ReturnStmt *stmt)
{
    ...
    {
        Pulse::error(stmt->keyword, "Cannot return from top-level code.");
    }

    if (current_function == INITIALIZER && stmt->value)
    {
        Pulse::error(stmt->keyword, "Cannot return a value from initializer.");
    }
